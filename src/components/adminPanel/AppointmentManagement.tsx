...
import { AppointmentResponseDto, AppointmentCreateDto, AppointmentUpdateDto } from '../../types/appointment';
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (selectedAppointment) {
        await dispatch(updateAppointment({ id: selectedAppointment.id, data: formData as AppointmentUpdateDto })).unwrap();
        setSnackbar({ open: true, message: 'Запись успешно обновлена', severity: 'success' });
      } else {
  ...
  const handleDelete = async () => {
    if (selectedAppointment) {
      try {
        ...
        setSnackbar({ 
          open: true, 
          message: selectedAppointment.appointmentStatus === AppointmentStatus.AVAILABLE 
        ...
        console.error('Error deleting or cancelling appointment:', err);
        setSnackbar({ 
          open: true, 
          message: `Не удалось ${selectedAppointment.appointmentStatus === AppointmentStatus.AVAILABLE ? 'удалить запись' : 'отменить бронирование'}. ${err}`, 
          severity: 'error' 
        });
      }
    }
  ...
  return (
    <LocalizationProvider dateAdapter={AdapterDateFns}>
      <Paper style={{ padding: '20px', margin: '20px auto' }}>
        ...
        <Dialog open={isDialogOpen || isUpdateDialogOpen} onClose={handleCloseDialog}>
          <DialogTitle>{selectedAppointment ? 'Обновить запись' : 'Создать новую запись'}</DialogTitle>
          <form onSubmit={handleSubmit}>
            ...
            <DialogActions>
              ...
              <Button type="submit" color="primary">
                {selectedAppointment ? 'Обновить' : 'Создать'}
              </Button>
              {selectedAppointment && (
                <Button onClick={() => setIsDeleteDialogOpen(true)} color="error">
                  Удалить
                </Button>
              )}
            </DialogActions>
